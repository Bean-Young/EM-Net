...
class Embeddings(nn.Module):
    ...
    def forward(self, x,y):
        if self.hybrid:
            x, features = self.hybrid_model(x,y)
        else:
            features = None
...
class Transformer(nn.Module):
    ...
    def forward(self, input_ids,y):
        embedding_output, features = self.embeddings(input_ids,y)
...
from .concat_1 import MM_2Block
class DecoderBlock(nn.Module):
    def __init__(
        ...
        self.concat=MM_2Block(ch_1=in_channels, ch_2=skip_channels, drop_rate=0.0)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x,y):
        ...
        if y.size()[1] == 1:
            y = y.repeat(1,3,1,1)
        x, attn_weights, features = self.transformer(x, y)


        high_level_features = x
        x = self.decoder(x, features)
        logits = self.segmentation_head(x)
        return logits, high_level_features
